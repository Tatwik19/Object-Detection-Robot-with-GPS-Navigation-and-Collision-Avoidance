...
#             plt.show()
    ...
    def draw_results(self, img, boxes, confs, classes,class_name,BOUNDARY_TH):
        window_name = 'final results'
        
        #cv2.namedWindow(window_name)
        h,w =640,640#c=img.size
        overlay = img.copy()
        final = img.copy()
        ...
        for box, conf, cls in zip(boxes, confs, classes):
            ...
            h,w =640.0,640.0
            if cen<(0.4*w) and box_area_per<BOUNDARY_TH:
                box_pos='P'
            elif cen>(0.6*w) and box_area_per<BOUNDARY_TH:
                box_pos='S'
            else:
                box_pos='C'
...
#             if cls==6:
#                 my_pwm.start(random.randint(9,15))
            ...
            #print('detected..')
            #print(f'{cls_name},{round(conf,2)},{box_pos},Area:{box_area_per}')
            #robot.backward(0.15)
            # draw text
            cv2.putText(final, '%s %s %.2f' % (cls_name,box_pos, round(conf,2)), org=(x1, int(y1+10)), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.5, color=(255, 255, 255))
